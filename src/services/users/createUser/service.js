...
const result = await User.sequelize.transaction(async (t) => {
      ...
      if(body.directActiveFlag > 0 && body.statusId === 101){
        ...
        usrObj = {...usrObj, pwd: encryptedPassword, status_id: body.statusId}

        console.log(usrObj)
        const usret = await User.create(usrObj, { transaction: t });
        
        const rolesCreate = body.roles.map(obj => ({ role_id: obj, user_id: usret.id }))

        const rolesret = await RoleUser.bulkCreate(rolesCreate, { transaction: t });
        console.log(rolesret)

        const branchesCreate = body.branches.map(obj => ({ branch_id: obj, user_id: usret.id }))
      ...
      else{ //usuario dado de alta como Nuevo, llega email con pwd random
