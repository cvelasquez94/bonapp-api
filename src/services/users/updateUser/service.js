...
const result = await User.sequelize.transaction(async (t) => {
        ...
        if (body.branches){
          const branchesCreate = body.branches.map(obj => ({ branch_id: obj, user_id: user.id }))
          
          await user_branches.destroy({where: {user_id: user.id}}, { transaction: t });
          await user_branches.bulkCreate(branchesCreate, { transaction: t });
        }

        if (body.roles){
          const rolesCreate = body.roles.map(obj => ({ role_id: obj, user_id: user.id }))
          
          await RoleUser.destroy({where: {user_id: user.id}}, { transaction: t });
