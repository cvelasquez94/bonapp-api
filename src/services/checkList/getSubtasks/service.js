...
module.exports = (fastify) => {
  ...
  async function getSubTasks(roleid, branchid, limit, userId) {
      ...
      const checkLists = await Checklist.findAll({
        ...
        include: [{
          ...
          include: [{
            model: SubTask,
            as: 'subTasks',
            include: [{ // Incluye el modelo STaskInstance aquí
              model: STaskInstance,
              as: 'sTaskInstances',
              where: {
      ...
      checkLists.forEach(checklist => {
        checklist.mainTasks.forEach(mainTask => {
          mainTask.subTasks.forEach(subTask => {
            ...
            subTask.dataValues.status = subTask.sTaskInstances?.length > 0 ? subTask.sTaskInstances[0].status : null;
    ...
    subTasks.sort((a, b) => {
      if (a.dataValues.status === 'completed' && b.dataValues.status !== 'completed') {
        return 1; // Mueve la tarea a hacia el final
      } else if (a.dataValues.status !== 'completed' && b.dataValues.status === 'completed') {
